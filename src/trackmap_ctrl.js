...
export class TrackMapCtrl extends MetricsPanelCtrl {
  ...
  setupMap() {
    ...
    this.leafMap = L.map('trackmap-' + this.panel.id, {
      ...
      L.marker(e.latlng).bindPopup(`<div class='display: flex;flex-direction: column;'><p>${e.latlng.lat} , ${e.latlng.lng}</p><p>distance: ${distance(this.leafMap, this.vesselPosLst[this.vesselPosLst.length - 1].position, e.latlng)}</p><input type='button' value='Get Location' class='marker-location-button'/><input type='button' value='Delete this marker' class='marker-delete-button'/></div>`).on("popupopen", function() {
